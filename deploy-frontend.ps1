...
# =============================================================================
# deploy-frontend.ps1 -- Build and deploy the frontend to Cloud Run
#
# Reads all environment-specific values from gcp.config.ps1.
# BACKEND_URL must be set in gcp.config.ps1 before running this script.
# =============================================================================
...
if ($BACKEND_URL -eq "https://your-backend-url.a.run.app" -or $BACKEND_URL -eq "") {
    Write-Host ""
    Write-Host "ERROR: Set BACKEND_URL in gcp.config.ps1 to the backend Cloud Run URL." -ForegroundColor Red
    Write-Host "       Run deploy-backend.ps1 first and copy the printed URL." -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
...
$SERVICE_NAME  = "customer-ops-frontend"
$IMAGE_NAME    = "customer-ops-frontend"
$PORT          = "80"
$MEMORY        = "256Mi"
$CPU           = "1"
$MIN_INSTANCES = "0"
...
Write-Host "[5/6] Building and pushing image (VITE_API_URL=$BACKEND_URL)..." -ForegroundColor Yellow
docker build --platform linux/amd64 --build-arg "VITE_API_URL=$BACKEND_URL" -t "$IMAGE_TAG" "$FRONTEND_DIR"
...
Write-Host ""
Write-Host "Frontend URL: $SERVICE_URL" -ForegroundColor Green
Write-Host ""
Write-Host "If this is your first deploy, copy the URL above into CORS_ALLOW_ORIGINS" -ForegroundColor Yellow
Write-Host "in gcp.config.ps1, then run deploy-backend.ps1 again to apply it." -ForegroundColor Yellow
Write-Host ""
